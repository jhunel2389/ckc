...
@section('content')
                    ...
                    <tr>
                      ...
                      <td>
                        ...
                        <form action="{{ route('updateTeam') }}" method="POST" id="formUpdateStatus">
                          ...
                          <button type="button" class="btn btn-xs" onclick="openBookmarks('{{$value['id']}}')"><i class="fa fa-bookmark" title="Bookmarks"></i></button>
                        </form>
                      </td>
                    </tr>
                  @endforeach
    ...
    @if($utils::checkPermissions('edit_bookmarks'))
      <!-- ADD MODAL -->
      <div class="modal fade" id="modal-bookmarks">
          <div class="modal-dialog modal-lg">
            <div class="modal-content" id="modal-content-tools">
              <div class="modal-header">
                <h4 class="modal-title" id="modal-h4">Bookmarks Management</h4>
                <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                  <span aria-hidden="true">&times;</span>
                </button>
              </div>
                    ...
                    <div class="form-group">
                      <div class="row">
                        <div class="col-md-12">
                          ...
                          <input type="text" class="form-control" id="bm_name" name="bm_name" value="" placeholder="Enter tools name" required>                        
                        </div>
                        <div class="col-md-12">
                          <label for="bm_description">Description</label>
                          <input type="text" class="form-control" id="bm_description" name="bm_description" placeholder="Enter team description" value="">
                        </div>
                        <div class="col-md-12">
                          <label for="bm_link">Link  <span style="color:red;">(Please include http or https)</span></label>
                          <input type="text" class="form-control" id="bm_link" name="bm_link" placeholder="Enter Training Link" value="">
                        </div>
                        <div class="col-md-12">
                          <br>
                          <button type="button" class="btn btn-block btn-success" onclick="addBookmarkTeam()">Add</button>
                        </div>
                      </div>
                    </div>
                    <div class="form-group">
                      <div class="card">
                        ...
                        <div class="card-body">
                          <table id="bookmarks-table" class="table table-bordered">
                            <thead>
                            <tr>
                              <th>Name</th>
                              <th>Description</th>
    ...
    @section('custom_script')
      <script>
        ...
        function openBookmarks(team_id){
          $('#er_id').val('');
          $('#er_name').html('');
          $('#team_id').html('');
          $('#team_id').val(team_id);
        ...
        function addBookmarkTeam(){
          $.post("{{route('ajax.add-team-bm')}}", { name: $('#bm_name').val(),description: $('#bm_description').val(),link: $('#bm_link').val(), team_id: $('#team_id').val(), _token: "{{ csrf_token() }}" }, function(data, status){
            if(status === 'success'){
              alerts_float(data.alert_status,data.alert_msg,data.alert_class);
              refreshAjaxCall();
            }
        ...
        function loadTeamBookmarkTable() {
          $("#bookmarks-table").DataTable().destroy();
          $("#bookmarks-table").DataTable(
            {
                processing: true,
                serverSide: true,
                autoWidth: false,
                ajax: {
                  url: '{!! route('datatables.team-bookmark') !!}',
                  type: 'GET',
                  data: function (d) {
                    d.team_id = $('#team_id').val();;
                  }
                ...
                columns: [
                    ...
                    { data: 'id', render: function (dataField) {
                     return '<button type="button" class="btn btn-xs" onclick="removeBookmark('+dataField+')"><i class="fa fa-trash" title="Tools"></i></button>'; }
                    }
                ]
            });
        }
        ...
        function removeBookmark(id){
          $.post("{{route('ajax.delete-team-bookmark')}}", { bm_id: id, _token: "{{ csrf_token() }}" }, function(data, status){
            if(status === 'success'){
              alerts_float(data.alert_status,data.alert_msg,data.alert_class);
              refreshAjaxCall();
            }
